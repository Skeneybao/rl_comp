...
feature_engine = FeatureEngine_single600T_Mod(max_position=max_position)
model = DNN(
...
checkpoint = torch.load(os.path.join(current_fp, '2000000.pt'))
...
def my_controller(observation, action_space, is_act_continuous=False):
    ...
    env_info_appender.accumulate((obs['ap0'] + obs['bp0']) / 2, (obs['ap0'] - obs['bp0']) / obs['ap0_t0'], obs['signal0'], obs['signal1'], obs['signal2'], 0)
