...
def evaluate_model(config: EvaluatorConfig):
    ...
    while env.reset_cnt <= len(env):
        ...
        if not obs['warming-up']:
            ...
            valid_action, is_invalid = validate_action(obs, action, max_position=feature_engine.max_position, signal_risk_thresh=config.explicit_config.signal_risk_thresh)
...
if __name__ == '__main__':
    ...
    config = EvaluatorConfig(
        ...
        date='ALL',
        training_res_path='/mnt/data3/rl-data/training_res/7u6e3hgm/h3X66',
        model_name='5100000.pt',
